...
export default class App extends Component {  
  ...
  componentDidMount(){
    this.setState({bgColor:this.randomizeColor(),gridColor:this.randomizeColor()})
  }
  ...
  checkDraw = () => {
    ...
    this.setState({winMessage:'Match is Tie'})
    return
  }

  drawItem = itemNumber => {
    this.setState({gridColor:this.randomizeColor()})
  ...
  resetGame = () => {
    ...
    this.setState({bgColor:this.randomizeColor()})
  ...
  winGame = () => {
    if((itemArray[0] !== 'empty') && (itemArray[0] === itemArray[1]) && (itemArray[1] === itemArray[2])){
      ...
      this.setState({ winMessage: (itemArray[1] ? 'cross' : 'circle').concat(' won') })      
    }else if((itemArray[2] !== 'empty') && (itemArray[2] === itemArray[5]) && (itemArray[5] === itemArray[8])){
      this.setState({ winMessage: (itemArray[2] ? 'cross' : 'circle').concat(' won') })
  ...
  return (
    <View style={[styles.container,{backgroundColor:this.state.bgColor}]}>
      <View style={[styles.grid,{backgroundColor:this.state.gridColor}]}>
